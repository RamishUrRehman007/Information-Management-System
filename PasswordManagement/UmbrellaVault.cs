...
namespace PasswordManagement
{
    ...
    public class LoginFor : UmbrellaVault
    {
        ...
        public void LoginButton(string loginusername, string loginpassword)
        {
            //try
              //  {
                    string constring = @"Provider=Microsoft.ACE.OleDb.12.0;Data Source=C:\Users\DELL\Documents\PasswordManagement.accdb;";
            OleDbConnection conDataBase = new OleDbConnection(constring);
            OleDbCommand cmdDataBase = new OleDbCommand("Select * from SignUpForm where Sign_Up_Email='" + loginusername + "' and Sign_Up_Password='" + loginpassword + "';", conDataBase);
    ...
    class UserDashboar : UmbrellaVault
    {
        ...
        public void DashUpdate(string strID)
        {
            string connstring = ConfigurationManager.ConnectionStrings["dbx"].ConnectionString;
            string strQuery = "Delete from EntryForm where Entry_Email=@Entry_Email";
            try
            {
                using (OleDbConnection con = new OleDbConnection(connstring))
                {
                    ...
                    {
                        con.Open();
                        cmd.Parameters.AddWithValue("@Entry_Email", strID);
                        cmd.ExecuteNonQuery();

                        MessageBox.Show("Record is deleted successfully.", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        con.Close();
                    }
                }
            }
    ...
    class EntryFor : UmbrellaVault
    {
        ...
        public void EntryCreate(string entrysitename, string entryusername, string entrypassword, string entryemail, string entrynumber)
        {
            try
            {
                ...
                string connstring = ConfigurationManager.ConnectionStrings["dbx"].ConnectionString;
                string strQuery = "INSERT INTO EntryForm (Entry_Site_Name,Entry_Username,Entry_Password,Entry_Email,Entry_Number,Entry_SignUpMail) values (@Entry_Site_Name,@Entry_Username,@Entry_Password,@Entry_Email,@Entry_Number,@Entry_SignUpMail)";
                ...
                using (OleDbConnection con = new OleDbConnection(connstring))
                {
                    using (OleDbCommand cmd = new OleDbCommand(strQuery, con))
                    {
                            ...
                            cmd.Parameters.AddWithValue("@Entry_SignUpMail", LoginFor.name());
